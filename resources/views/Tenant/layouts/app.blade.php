...
<head>
    ...
    <link rel="stylesheet" href="{{asset('theme/plugins/global/plugins.bundle.css')}}" type="text/css" />

    <link rel="stylesheet" href="{{asset('theme/css/style.bundle.css')}}" type="text/css" />

    <link rel="stylesheet" href="{{asset('theme/plugins/custom/uppy/uppy.bundle.css')}}" type="text/css" />
    ...
    <link rel="stylesheet" href="{{asset('theme/css/skins/header/base/navy.css')}}" type="text/css" />
    <link rel="stylesheet" href="{{asset('theme/css/skins/header/menu/light.css')}}" type="text/css" />
    <link rel="stylesheet" href="{{asset('theme/css/skins/brand/navy.css')}}" type="text/css" />
    <link rel="stylesheet" href="{{asset('theme/css/skins/aside/navy.css')}}" type="text/css" />
    ...
    {{--------------------------------------
                CUSTOM FILES
    ---------------------------------------}}
    @include('Accounts.layout.account_head')
    @section('head')
...
<body class="kt-quick-panel--right kt-demo-panel--right kt-offcanvas-panel--right kt-header--fixed kt-header-mobile--fixed kt-subheader--enabled kt-subheader--transparent kt-aside--enabled kt-aside--fixed kt-aside-secondary--enabled kt-page--loading @if($helper_service->helper->getConfig()->sidebar!=1) kt-aside--minimize @endif">
            ...
            <div class="kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor kt-wrapper" id="kt_wrapper">
                ...
                <div class="kt-content  kt-grid__item kt-grid__item--fluid kt-grid kt-grid--hor m-0" id="kt_content">
                    ...
                    <div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
                        @include('Tenant.includes.message')
                        @yield('content')
                    </div>
                </div>
    ...
    <script src="{{asset('theme/plugins/custom/uppy/uppy.bundle.js')}}" type="text/javascript"></script>
    ...
    *****************************************************************************--}}
    ...
    <script type="text/javascript">
        ...
        *****************************************************************************/
        ...
        var kingriders={
            ...
            Config:{
                storage_path: "{{ $helper_service->helper->storage_basepath() }}"
            },
            Utils:{
        ...
        *****************************************************************************/
        $(function(){
            // Handle Log Activity ALT + CTRL + CLick Event
            @if ($helper_service->routes->has_access('tenant.admin.log_activity.view'))
            $(document).on('click','[data-activity-modal]', function(event){
                if((event.ctrlKey || event.metaKey) && event.altKey){
                    let id = $(this).attr('data-activity-id');
                    let classname = $(this).attr('data-activity-modal');
                    let category = $(this).attr('data-activity-category') ?? 'default';
                    $('#log-activity-action').attr('kr-ajax', (idx, val) => {
                        val = `{{route('tenant.admin.log_activity.view')}}?id=${id}&modal=${classname}&category=${category}`;
                        return val;
                    });
                    $('#log-activity-action').click();
                }
            })
            @endif
    ...
    {{--
        --------------------
        TASK OFF-CONVAS PANEL
        --------------------
    --}}
    @include('Tenant.tasks.frontend.partials.tasks_embed')
    ...
    {{-- Renders notificaitons like: license/visa expiring + pending cheques alerts --}}
    {{-- @include('Tenant.includes.header_notifications') --}}
